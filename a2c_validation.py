...
def validation_ensemble(model, data_loader, device, step_num, save_to, TASK:str, restore=True):
    print(f'Start Validation for Public Score')
    with torch.no_grad():
        for step, (images, fnames, sizes) in enumerate(data_loader, 1):
            printProgress(step, step_num, 'Progress:', 'Complete', 1, 50)
            images = torch.stack(images)  # (N, C, W, H)
            ...
            if restore:
                outputs = np.expand_dims(outputs, -1)
                outputs = [A.Resize(h, w, always_apply=True)(image=output.astype(np.float32))['image']
                        for output, (w, h) in zip(outputs, sizes)]
...
def generate_valset_predictions(val_a2c_path, batch_size, encoder_type):

    val_a2c_df = make_data_df(val_a2c_path)

    val_a2c_dataset = CustomDataset(df=val_a2c_df, mode='test', transform=val_transform(768), has_masks=False)
    val_a2c_loader = torch.utils.data.DataLoader(
                            dataset=val_a2c_dataset, 
    ...
    a2csteps = (len(val_a2c_dataset)//batch_size)+1
    a2c_save_to = './val_pred/A2C/'

    validation_ensemble(a2c_models, val_a2c_loader, device, a2csteps, a2c_save_to, 'A2C', restore=True)
    
    print('Generated Predictions of Validation Set !!!')

def val_score_pred_and_mask(val_a2c_mask_path, val_a2c_pred_path, batch_size):

    # A2C 정답 및 예측 Mask 파일들의 경로 DataFrame 생성    
    val_a2c_mask_df = make_mask_df(val_a2c_mask_path)
    val_a2c_pred_df = make_mask_df(val_a2c_pred_path)

    val_a2c_score_dataset = ScoreDataset(mask_df=val_a2c_mask_df,
                                        pred_df=val_a2c_pred_df)
    val_a2c_score_loader = torch.utils.data.DataLoader(
                            dataset=val_a2c_score_dataset, 
                            batch_size=batch_size,
                            num_workers=4,
                            collate_fn=collate_fn,
                            )

    a2c_cnt = len(val_a2c_score_dataset)
    a2c_dsc_sum = 0.
    a2c_ji_sum = 0.

    for a2c_masks, a2c_preds in val_a2c_score_loader:
    ...
    print(f'<A2C Validation set> DSC : {round(a2c_dsc, 4)}, JI: {round(a2c_ji, 4)}') 
...
if __name__ == '__main__':
    ...
    generate_valset_predictions(val_a2c_path, BATCH_SIZE, ENCODER_TYPE)

    # Validation Set 예측 Mask와 정답 Mask 간 Score 계산
    val_a2c_mask_path = './heartdata/validation/A2C'
    val_a2c_pred_path = './val_pred/A2C'

    val_score_pred_and_mask(val_a2c_mask_path, val_a2c_pred_path, BATCH_SIZE)
